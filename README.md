...
- Session maximum length can be set per-session using the proprietary `sessmax` property in a profile's configuration in the `config` file; when the effective IAM policy allows querying profile details, the `sessmax` is set automatically when it differs from the AWS default of 3600 (1h). When the IAM policy doesn't allow retrievals of the role configuration (such as perhaps with third-party roles), the value can be set manually by the user; this is important especially in cases where the allowed session maximum length is below 3600 seconds (1h), as the role session requests with a longer session time than what the role's policy allows, fail.
...
- When `region` and `output` properties have not been set for a profile, the script attempts to automatically inherit them first from the MFA/role session's source profile, and then from the `default` profile (if one is present).
...
- Provides the "quick mode" (use `--quick`/`-q` to activate) that forgoes the validity checks but brings up the menu much faster; this makes the utility a lot more convenient to use especially when you have lots of profiles configured and need to jump between baseprofiles or active sessions. However, I recommend executing the full mode every once in a while, and especially whenever there have been any changes to your profiles/IAM accounts as the full mode persists some of its findings to be used in the quick mode.
- Remaining validity periods are displayed for all MFA and role sessions that were started with this script. This feature is avialable both in full and quick mode.
...
- Provides the option to either persist a started MFA/role session into the `credentials` file, or to only export the session into the environment. When executed in WSL bash, it also provides profile/session activation strings for Windows PowerShell and Windows Command Prompt.
...
- The script validates the baseprofiles and the MFA/role sessions (full mode only); for the valid sessions the remaining time is provided if the sessions were started using this script. When the script detects an invalid baseprofile or an MFA/role session, it tags them as "invalid" in the `config` file with the last detection time (provided in UTC). The `invalid_as_of = {date stamp}` pseudo-property makes it easier to detect and remove the invalid profiles from the configuration when editing the configuration files.
...
- Provides a simplified interface when only a single profile plus its possible MFA session are present.
...
When the presence of a multi-factor authentication session is enforced with an IAM policy, the enforcement cannot be limited to the web console operations (nor should it, but that's a different topic). This is because the AWS web console is basically a front-end to the AWS APIs, i.e. the same ones which can also be accessed using the `aws` CLI command. When you log in to the web console and enter an MFA code, the browser takes care of caching the credentials and the session token, and so beyond that point, in the web browser, the MFA/role session is transparent to the user until the session eventually expires, and the AWS web console prompts the user to log in again. On the command line it's different. To create, enable, or disable a virtual MFA device (vMFAd), or to start an MFA or a role session, complex sequences of commands are required, followed by the need to painstakingly save the session token/credentials in the `~/.aws/credentials` file, and then either refer to that session profile by using the `--profile {session profile name}` switch on each `aws` CLI command, or add/modify/delete various `AWS_*` environment variables by cut-and-pasting at least the key id, the secret key, and the session token. Furthermore, the only way to know that a session has expired is that the `aws` CLI commands start failing, thus making it difficult to plan long-running command execution, and potentially being confusing as to why such failures should occur.
