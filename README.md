...
5. If you need to switch between the configured base profiles and/or active MFA or role sessions, re-execute `awscli-mfa.sh` and follow its prompts. If you need to disable/detach (and possibly delete) a vMFAd from an account, re-execute `enable-disable-vmfa-device.sh` and follow its interactive guidance.
...
- Supports chained roles which can only be authenticated with an existing role session
- External ID is supported in the roles
- Session maximum length can be set per-session using the proprietary `sessmax` property in the profile configuration in the `config` file; when the IAM policy so allows, the `sessmax` is set automatically when it differs from the AWS default of 3600. When the IAM policy doesn't allow retrieving the role configuration (such as perhaps with third party roles), the value can be set by the user; this is important especially when the allowed session maximum length is below 1h, or 3600 seconds, as the role session requests with longer session time than what the role policy allows, fail
- Validates profiles when executed in the full mode (use `--full`/`-f` or give no option as this is the default mode)
- Provides a "quick mode" (use `--quick`/`-q`) that forgoes the validity checks but brings up the menu much faster; this makes the utility a lot more convenient to use especially when you have lots of profiles configured. I recommend executing the full mode every once in a while, and especially whenever there have been any changes to your profiles/IAM accounts as the full mode persists some of its findings to be used in the quick mode
- Doesn't require the `default` profile to be configured in case you prefer to always explicitly define the profile you want to use with the `aws` command
- Written to be portable; the script is environment-aware in macOS (bash 3.2 - 5.0), Ubuntu/Debian, RedHat/CentOS, and Windows Subsystem for Linux ("WSL") with Ubuntu. The script is bash-native, but also has been tested with zsh.
- Provides the options to persist MFA/role sessions into the `credentials` file, or only export to the environment. When executed in WSL bash, it also provides profile/session activation strings for Windows PowerShell and Windows Command Prompt
- Provides a single-command prefix string for the selected profile (bash only)
- Automatically copies the selected activation string on the clipboard; this is a built-in in macOS and WSL bash; in the Linux Desktop distros (e.g., Ubuntu) `xclip` is required for automatic copy-to-clipboard
- The script validates the aws configuration files for common configuration errors and provides guidance on how to fix them, including providing source profile selection for roles which don't have a source profile configured (full mode only)
- The script validates the baseprofiles and MFA/role sessions (full mode only); for valid sessions the remaining time is provided if the sessions were started using this script. When the script detects an invalid baseprofile or an MFA/role session, it tags them as "invalid" in the `config` file with the last detection time (provided in UTC). This makes it easier to remove the invalid profiles from the configuration editing the configuration files. 
- Checks the current state of the environment on startup; if a profile/session is detected in the environment, its status and details are printed out
- Provides a simplified interface when only a single profile (+ a possible MFA session) is present in the configuration
- The only hard dependency is a recent version of the `aws` command. The script checks for the presence of the `aws` and its version, and instructs the user to install/upgrade if necessary. Additionally, in Linux, `coreutils` is required (and checked for), but it is generally part of the standard Linux installations
- The only soft dependency is `jq`. The script works without it, but with `jq` present it works faster, especially when any role profiles are present. When `jq` is not detected, the script will recommend it to be installed.

### Rationale

When the presence of a multi-factor authentication session is enforced with an IAM policy, the enforcement cannot be limited to the web console operations (nor should it, but that's a different topic). This is because the AWS web console is basially a front-end to the AWS APIs, the same ones which can also be accessed using the `aws cli`. When you log in to the web console and enter an MFA code, the browser takes care of caching the credentials and the session token, and so beyond that point, in the web browser, the MFA/role session is transparent to the user until the session eventually expires and the AWS web console prompts the user to log in again. On the command line it's different. To create, enable, or disable a virtual MFA device (vMFAd), or to start an MFA or a role session, complex sequences of commands are required, followed by the need to painstakingly save the session token/credentials in the `~/.aws/credentials` file, and then either refer to that session profile by using the `--profile` switch on each `aws cli` command, or add/modify/delete various `AWS_*` environment variables by cut-and-pasting at least the key id, the secret key, and the session token. Furthermore, the only way to know that a session has expired is that the `aws cli` commands start failing, thus making it difficult to plan long-running command execution, and potentially being confusing as to why such failures should occur.
...
* [SCRIPT NOT YET UPDATED FOR v2] **mfastatus.sh** - Displays the currently active MFA sessions and their remaining activity period. Also indicates expired persistent (or in-environment) profiles with "EXPIRED" status.
